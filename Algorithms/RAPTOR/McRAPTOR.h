...
private:
    ...
    inline void arrivalByTransfer(const StopId stop, const Label& label) noexcept {
        ...
        Assert(bestLabels[stop].byTransfer().dominates(currentRound()[stop]), "Best bag does not dominate current bag!");
        stopsUpdatedByTransfer.insert(stop);
    }
...
public:
    McRAPTOR_prune(const Data& data, const Profiler& profilerTemplate = Profiler()) :
        data(data),
        bestLabels(data.numberOfStops()),
        stopsUpdatedByRoute(data.numberOfStops()),
        stopsUpdatedByTransfer(data.numberOfStops()),
    ...
    template<typename ATTRIBUTE>
    McRAPTOR_prune(const Data& data, const InitialTransferGraph&, const InitialTransferGraph&, const ATTRIBUTE, const Profiler& = Profiler()) :
        McRAPTOR_prune(data) {
    }
        ...
        for (const StopId stop : stopsUpdatedByRoute) {
            ...
            for (size_t i = 0; i < bag.size(); i++) {
                for (const Edge edge : data.transferGraph.edgesFrom(stop)) {
                    ...
                    newLabel.transferId = edge;
                    arrivalByTransfer(toStop, newLabel);
                    if (bestLabels[targetStop].byTransfer().dominates(newLabel)) {
                        break;
                    }
                }
            }
