...
public:
    ...
    DijkstraRAPTOR(const Data& data, const CH::CH& chData, const Profiler& profilerTemplate = Profiler()) requires std::same_as<InitialTransferGraph, CHGraph> :
        DijkstraRAPTOR(data, chData.forward, chData.backward, Weight, profilerTemplate) {
    }

    DijkstraRAPTOR(const Data& data, const TransferGraph& forwardGraph, const TransferGraph& backwardGraph, const Profiler& profilerTemplate = Profiler()) requires std::same_as<InitialTransferGraph, TransferGraph> :
...
private:
    ...
    inline void scanRoutes(const ARRIVAL_TIME& arrivalTime) noexcept {
        ...
        for (const RouteId route : routesServingUpdatedStops.getKeys()) {
            ...
            Assert(stopIndex < tripSize - 1, "Cannot scan a route starting at/after the last stop (Route: " << route << ", StopIndex: " << stopIndex << ", TripSize: " << tripSize << ", RoundIndex: " << rounds.size() - 1 << ")!");

            const StopId* stops = data.stopArrayOfRoute(route);
            const StopEvent* trip = data.lastTripOfRoute(route);
            StopId stop = stops[stopIndex];
            Assert(trip[stopIndex].departureTime >= arrivalTime(stop), "Cannot scan a route after the last trip has departed (Route: " << route << ", Stop: " << stop << ", StopIndex: " << stopIndex << ", Time: " << arrivalTime(stop) << ", LastDeparture: " << trip[stopIndex].departureTime << ", RoundIndex: " << rounds.size() - 1 << ")!");
...
public:
    DijkstraRAPTOR_prune(const Data& data, const InitialTransferType initialTransfers, const Profiler& profilerTemplate = Profiler()) :
        data(data),
        initialTransfers(initialTransfers),
        earliestArrivalByRoute(data.numberOfStops() + 1, never),
        stopsUpdatedByRoute(data.numberOfStops() + 1),
    ...
    template<typename ATTRIBUTE>
    DijkstraRAPTOR_prune(const Data& data, const InitialTransferGraph& forwardGraph, const InitialTransferGraph& backwardGraph, const ATTRIBUTE weight, const Profiler& profilerTemplate = Profiler()) :
        DijkstraRAPTOR_prune(data, InitialTransferType(forwardGraph, backwardGraph, data.numberOfStops(), weight), profilerTemplate) {
    }

    DijkstraRAPTOR_prune(const Data& data, const CH::CH& chData, const Profiler& profilerTemplate = Profiler()) requires std::same_as<InitialTransferGraph, CHGraph> :
        DijkstraRAPTOR_prune(data, chData.forward, chData.backward, Weight, profilerTemplate) {
    }

    DijkstraRAPTOR_prune(const Data& data, const TransferGraph& forwardGraph, const TransferGraph& backwardGraph, const Profiler& profilerTemplate = Profiler()) requires std::same_as<InitialTransferGraph, TransferGraph> :
        DijkstraRAPTOR_prune(data, forwardGraph, backwardGraph, TravelTime, profilerTemplate) {
    }
    ...
    inline void dijkstra() noexcept {
        ...
        while (!queue.empty()) {
            ...
            for (Edge edge : data.transferGraph.edgesFrom(u)) {
                ...
                if (v == targetVertex || v == uLabel->parent) continue;
                profiler.countMetric(METRIC_EDGES);
                const int arrivalTime = uLabel->arrivalTime + data.transferGraph.get(TravelTime, edge);
                // Pruning check
                if (TargetPruning && arrivalTime > targetLabel.arrivalTime) {
                    break;
                }
                arrivalByEdge<true>(label[v], arrivalTime, uLabel->parent);
            }
            if constexpr (UseMinTransferTimes) {
                for (Edge edge : minChangeTimeGraph.edgesFrom(u)) {
                    const Vertex v = minChangeTimeGraph.get(ToVertex, edge);
